...
<div class="card mb-3 " >
  ...
  @endif
    ...
    By. <a href="/posts?author={{ $posts[0]->author->username }}">{{ $posts[0]->author->name }}</a> in <a href="/posts?category={{ $posts[0]->category->slug }}" >{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }} </
      ...
      <a href="/posts/{{ $posts[0]->slug }}" class="text-decoration-noe btn btn-primary">Read more</a>
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-4 mb-3">
        ...
        <div class="card">
          <div class="position-absolute px-3 py-2 text-white " style="background-color: rgba(0, 0, 0, 0.399)"><a href="/posts?category={{ $post->category->slug }}" class="text-white text-decoration-none">{{ $post->category->name }}</a></div>
          ...
          <div class="card-body">
            <h5 class="card-title"><a href="/posts/{{ $post->slug }}" class="text-black text-decoration-none">{{ $post->title }}</a></h5>
